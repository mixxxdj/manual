...
Start <#1-and-2-(Track-select-buttons)>`__ is enabled the deck will stop at the
previously used Cue if the Volume Fader reaches the minimum position and will
start playing if the Volume fader moves from the minimum position.
...
-  **Eject:** Hold the same button for more than half of a second to unload the
   same deck.
-  **Fader Start:** Hold `SHIFT <#SHIFT>`__ down and then press one of these
...
-  \**Slip mode: \**Hold `SHIFT <#shift>`__ down and then press these buttons to
...
-  **Quantize mode:** Hold `SHIFT <#shift>`__ down and then press these buttons
...
-  **End track warning:** Hold `SHIFT <#SHIFT>`__ down and then press this
   button in order to toggle **on** or **off** the red flashing of the `jog
   wheels <#JOGWHEEL>`__ when the playback reaches the end of the track.
...
-  `SHIFT <#shift>`__\ **\ +Turn** to select a folder or subfolder in the left
   item list sidebar.
-  `SHIFT <#shift>`__\ **\ +Push** to open/close folders and subfolders in the
...
-  **Play/pause:** Press to play/pause the track. If there was no track loaded
   ...
   playing.
-  **Censor:** Hold `SHIFT <#shift>`__ down and then press this button to play
...
-  **Brake:** Hold `SHIFT <#shift>`__ down and then press this button to stop
   ...
   speed.
-  **Spinback:** see the `Instant FX <#Instant-fx>`__ usage.
...
-  **While stopped:** Sets the cue point (Pioneer/Mixxx mode) OR preview from it
   ...
   hold the **SET** Button, then press and hold the `Play/Pause <#Play/Pause>`__
...
-  **Key lock:** Hold `SHIFT <#shift>`__ down and then press this button to
...
-  **iCut mode:** Hold `SHIFT <#shift>`__ down and scratch to scratch in
...
| Deactivate both the `Scratch mode <#Scratch-mode>`__ and the `Seek
  mode <#Seek-mode>`__ to enable this mode.
...
-  \**Beat Jump: \**Hold `SHIFT <#shift>`__ down and then use these buttons to
...
(see also `Bounce Loop (Roll) mode <#Bounce-Loop-(Roll)-mode>`__ section)
...
-  **Loop size:** Turn the encoder to select the number of beats for a loop.
   ...
   it.
-  **Loop move:** Hold `SHIFT <#shift>`__ down and then use the encoder to move
   the triggered loop forward or backwards by 1 beat.
-  **Activate/deactivate:** (Acts exactly like the `LOOP <#loop>`__ Button)
...
-  **Roll loop mode:** Press (and hold down) to trigger a momentary rolling loop
   ...
   never enabled. In other words, this is a regular loop in `slip
   mode <#PFL-(symbolized-by-a-headphone)>`__ (see
   `PFL <#PFL-(symbolized-by-a-headphone)>`__ section).
-  **Toggle loop mode:** Hold `SHIFT <#shift>`__ down and then push the encoder
   to toggle between Regular and Roll Loop mode.

Effects Section
---------------

(see also `Instant FX mode <#Instant-FX-mode>`__ and `Sampler
mode <#Sampler-mode>`__ sections)
...
-  **Temporary effect:** Push and hold the encoder to temporarily apply the
   ...
   pressed).
-  **Quick Effect:** Hold `SHIFT <#​shift>`__ down and then push the encoder
...
-  **MIX:** Hold `SHIFT <#​shift>`__ down and then use this knob to control the
...
-  `SHIFT <#SHIFT>`__\ **\ +Press** to delete its assigned Hot Cue Point.
...
Roll <#Good-to-know-for-beginners>`__ of a different size (in beats) as per the
table.
...
Loop Roll can be adjusted with the `LOOP SIZE <#LOOP-SIZE>`__ encoder as well.
Pads change the “selected loop size” introduced in `LOOP SIZE <#LOOP-SIZE>`__
and `LOOP <#LOOP>`__ sections.
...
-  **Spinback:** Hold `SHIFT <#shift>`__ down and then press this button down in
   order to stop the track with a backward brake effect. If the `Instant
   FX <#Instant-fx>`__ button is released before the track has completely
   stopped, the track is then played back to its regular speed, in the forward
   direction.
-  **Brake :** see `JUMP <#jump>`__ button usage
...
-  **Instant FX:** Press (and keep down) any of the 4 pads to apply momentarily
   ...
   can be adjusted from the `FX PARAM <#FX-PARAM>`__ knob.
-  **Current effect rack selection:** Hold `SHIFT <#shift>`__ down and then
   ...
   triggered later on (see `EFFECTS SECTION <#EFFECTS-SECTION>`__).
   this can also be done with a visual feedback in `Sampler
   mode <#Sampler-mode>`__

Sampler mode
~~~~~~~~~~~~

**Note:** On account of the Reloop Beatpad limitations (bug ?), lights will not
show on in Sampler mode when the controller is in `SHIFT <#shift>`__ mode.
