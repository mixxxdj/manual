...
iTunes, Traktor, Serato, Rhythmbox, Banshee - Using external libraries
======================================================================
...
From Mixxx 2.3, tracks in your Serato library and crates can now be imported into Mixxx library table.
Tracks can also be loaded into deck without needing to add the music directory in the preferences. 
You can also import your Serato library into Mixxx that are in USB drives for a portable Serato library.
...
* Click on Serato in the library, all records on USB from Serato will be visible (attach as many as you like!)
* Then click on the desired removable device, and all your folders and playlists from Serato will be revealed for you to play from

Import your Beatgrid and Hotcues
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Track cue points are saved in special file tags as well as the track metadata. 
You can now import your track cue points and all of it's metadata from Serato to Mixxx.

Once you import your Serato library into Mixxx and load a track for the first time, the positions, labels and colors of your hotcues, other metadatas and beatgrid are automatically imported.
...
Decodes may slightly detect different track start and end times for files from different sources, this could possibly be because of the presence of countless encoders and decoders for MP3 and M4A/AAC.
This problem may cause your cues to be shifted up by a few milliseconds. 
While we are working very hard to mitigate this problem, we have added a way to shift all cues for a track at once. 
With this method, it possible to fix the cue positions if Mixxx fails to determine the correct offsets.

Below are lists of tags that are currently read and parsed by Mixxx
...
+------------+------------+-----------+-----------+---------------+---------------------------+
| Beatgrid   | Yes        | Yes       | Yes       | No            | Beatgrid Markers          |
+------------+------------+-----------+-----------+---------------+---------------------------+
| Markers    | Yes        | N/A       | Yes       | No            | Hotcues, Saved Loops, etc |
+------------+------------+-----------+-----------+---------------+---------------------------+
...
From Mixxx 2.3, saved loops from Serato can be imported, but only the first loop is usable in Mixxx 2.3. 
Proper support for importing and restoring saved loops from Serato is scheduled for Mixxx 2.4. 

For now, we just import these loops into the Mixxx database and allow using their start position as regular hotcues. 
With this in place, you can start using your loops once Mixxx 2.4 is out and avoid reimporting track meta data.

Imports not currently supported
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Support for importing track overview waveform image stored in Serato into Mixxx and smart crates has not been provided in Mixxx 2.3. 
Serato flip macros cannot be imported either because Mixxx does not offer that kind of functionality yet.

Currently, you cannot write cues from Mixxx into Serato metadata tags. 
We are working hard to implemment support for exporting Serato metadata tags in upcoming Mixxx versions, stay tunned.
