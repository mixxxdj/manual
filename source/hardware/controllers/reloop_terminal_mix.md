...
![Terminal Mix 2](_static/controllers/reloop/terminalmix2_top.jpg)
![Terminal Mix 4](_static/controllers/reloop/terminalmix4_top.jpg)
...
The output level of both master outputs, booth and headphone outputs are
...
The jog wheels' touch sensitivity can be adjusted separately. Refer to
the [official
...
standard parts and relatively easy to replace, and the sliders'
...
*The below controls are not deck-specific.*
  ...
  - **TRAX knob**
    ...
    * Press while Tree panel has focus: expand an item if possible. If
    item can't be expanded further, it is selected and and highlight is
    ...
    * Press while tracks table has focus: perform the Track Load Action
    ...
    empty deck)
  - **Volume Level Meter**
    * LEDs display the hardware(\!) volume of the Master output (2 blue
  ...
  - **MIDI** LED (blue)
    ...
    * when it flashes although no control is touched, it's probably due to
    dust in the jogwheels (See [Controller Hardware /
