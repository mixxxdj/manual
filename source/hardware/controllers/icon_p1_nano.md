...
-  [Manufacturer's product page](https://iconproaudio.com/product/p1-nano/)
-  [Forum thread](https://mixxx.discourse.group/t/icon-pro-audio-p1-nano/31630)
...
:::{note}
Device support was tested with the Icon P1-Nano, but may also work
...
[D-5](https://iconproaudio.com/product/d5/) extension screens and displays
...
* - Master
  ...
  - Gain [^2]
```
[^1]: The zero value of the tempo faders is in the middle of the controller.
[^2]: Because the master fader controls gain and not volume, the zero value is
...
* - Solo
  - {hwlabel}`S`
  - Move beatgrid to current position
* - Record Arm
  - {hwlabel}`⏺`
  - BPM tap
* - Shift
...
* - Main
  - {hwlabel}`Master (PT-Ctrl)`
  - Toggle the screen and fader to the main channel gain/vumeter.
* - Circles
  - –
  - The circle buttons are not mappable and control the touch screen controls.
* - Lock
  - {hwlabel}`🔓`
...
:::{note}
Currently Mixxx must be focused for the library navigation buttons to
...
* - Function
  - Name
  - Function
* - Jog Wheel (deck playing)
  - –
  - Speed up / slow down the track
* - Jog Wheel (deck paused)
  - –
  - Seek in the track
* - Jog Wheel (push)
  - –
  - Select item (load track, expand selected menu, etc.)
* - Move
  - {hwlabel}`🢑🢓`
  - Change the jog wheel to scroll up and down in the focused view.
* - Shuffle
  - {hwlabel}`🢐🢒`
  - Change the jog wheel to move left and right in the focused view.
* - Navi (illuminated)
  - {hwlabel}`Navi`
  - Change the jog wheel to send a keyboard left/right arrow press and release signal.
* - Navi (blinking)
  - {hwlabel}`Navi`
  - Change the active fader (same as the {hwlabel}`|<` and {hwlabel}`>|` buttons).
* - Focus
  - {hwlabel}`Focus`
  - Change the jog wheel to send a mouse wheel scroll signal.
* - Zoom Left/Right
  - {hwlabel}`🢐🢒`
  - Change the jog wheel to move the cross fader.
* - Zoom Up/Down
  - {hwlabel}`🢑🢓`
  - Change the jog wheel to select the focused library pane.
```

## Mapping options

Settings can be edited in the preference windows, under {guilabel}`Preferences` > {guilabel}`Controllers` > {guilabel}`iCON P1-Nano ...`.
...
* - Setting
  - Description
* - Disable VuMeters
  - Disables the on-screen volume meters.
* - Disable Timecode Display
  - Disable the 7-segment timecode display.
* - Blink record arm button on beat
  - Blinks the BPM tap button on the beat.
```
