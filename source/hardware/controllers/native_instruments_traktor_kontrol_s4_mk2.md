...
On Windows, it is recommended to install the [driver from Native Instruments](https://www.native-instruments.com/en/support/downloads/drivers-other-files/) and select the {ref}`ASIO sound API in the preferences <preferences-sound-api>`.
...
Settings can be edited in the preference windows, under {guilabel}`Preferences` > {guilabel}`Controllers` > {guilabel}`Traktor Kontrol S4 MK2 ...`.
