...
The easiest way to tell the MK3 apart from the other MK1 and Mk2 models are these screens, placed in between the "Move" and "Loop" encoders.
...
If you don't want to use certain outputs of the device, you can enable the mapping of the mixer knob to the Mixxx internal mixer. Head over to the {ref}`settings` to enable these optional mappings.
   ...
   :::{note}
   The mixer knobs are physically linked to the S4 embedded mixer. This means that there is no way to prevent these buttons to adjust gains of the output, and will lead to unexpected volume mixing if the Mixxx mixer is used while also using S4 outputs.
   :::
...
During the development phase, it appears that using other Sound API than _ALSA_ would lead the Master VU meter not to work. Furthermore, audio output power would be half as loud as it should
...
Haptic feedback (know as Haptic Drive (TM)) is partially implemented. Supported features include jogwheel tensions and turntable mode but these features remains in beta and
...
You can enable them by setting _UseMotors_ to `true` in {ref}`settings`.
...
3. **Turntable mode**: The wheel behaves as a 33.3 RPM turntable. If the platter or crown are slowed down or speeded up, then it will scratch down or up the track.
   ...
   :::{note}
   This mode is only available if the motor features are enabled. See {ref}`use-motors` for more details.
   :::
...
* - Loop in mode
  ...
  - The {hwlabel}`REV` button is blinking
* - Loop out mode
  - N/A
  - N/A
  - The whole circle is blinking
  - The {hwlabel}`FLX` button is blinking
```
...
* - Mode
  - How to select
* - Vinyl mode
  - Press the {hwlabel}`Jog` button (while the {hwlabel}`Jog` button is off)
* - Jog mode
  - Press the {hwlabel}`Jog` button while in vinyl mode (while the {hwlabel}`Jog` button is on)
* - Turntable mode
  - Press the {hwlabel}`TT` button. Pressing again while restore the Vinyl mode
* - Loop in
  - Press {hwlabel}`SHIFT` + {hwlabel}`REV` while a loop is enable
* - Loop out
  - Press {hwlabel}`SHIFT` + {hwlabel}`FLX` while a loop is enable
```
...
5. **Hotcue color**: When first holding {hwlabel}`HOTCUES`, then tapping a specific hotcue, the move encoder can be used to change the selected color.
   ...
   :::{note}
   When using the shared data API, this will display hotcue metadata such as label (if any), timecode and type.
   :::
...
* - Grid
  - This mode is enabled while {hwlabel}`GRID` is held down
* - BPM
  - This mode is enabled while {hwlabel}`SHIFT` + {hwlabel}`GRID` are held down
* - Keyboard
  - This mode is enabled while {hwlabel}`SHIFT` + {hwlabel}`STEM` is held down
* - Hotcue color
  - This mode is enabled after holding {hwlabel}`HOTCUES` for a short period of time
```
...
* - Jogwheel crown
  - - Jog when in Vinyl mode
    - Same as wheel platter otherwise
  -
* - Flux
  - - Toggle Slip mode
    - On shift, set a loop end at the current track position if no active loop, enable loop out wheel mode otherwise
  - - Steady on: Reverse enabled
    - Steady on while pressing {hwlabel}`SHIFT`: loop is active
    - Blinking, but flux steady off while pressing {hwlabel}`SHIFT`: loop in set wheel mode
    - Blinking, with flux steady on while pressing {hwlabel}`SHIFT`: loop in wheel mode active
* - Reverse
  - - Toggle reverse mode
    - On shift, set a loop in the current track position if no active loop, enable loop out wheel mode otherwise
  - - Steady on: Flux enabled
    - Steady on while pressing {hwlabel}`SHIFT`: loop is active
    - Blinking, with reverse steady on while pressing {hwlabel}`SHIFT`: loop out wheel mode active
* - Grid
  - - Set the beatgrid at the current track position (on release, short press)
    - Enable grid move mode while pressed
    - Enable BPM move mode while pressed and pressing {hwlabel}`SHIFT`
  - - On when over a detected beat
    - Blinking when grid/BPM move is on
    - Blinking when grid move mode is enabled
* - Turntable
  - - Toggle on or off the turntable mode
  - - On: Turntable mode on, otherwise jog or vinyl
* - Jog
  - - Toggle on or off the vinyl mode
  - - On: Vinyl mode on, otherwise jog or turntable
* - Deck Select
  - - Select a deck
  - - The deck's main color will be the one of the selected deck
* - Shift
  - Shift controls for the entire controller side, including effect unit
  - - On or Off
* - Cue
  ...
  - Depends on the {ref}`cue mode <interface-cue-modes>`
* - Play/Pause
...
* - Move
  - - Beat jump forward (right) or backward by the number of selected beats
...
* - Loop
  - - Enable/disable loop when pressed
    ...
    - On loop in/out wheel mode: move the loop with precision, left precision if shifted
  -
* - Master
  - - Make the current deck sync leader (on release)
...
* - Tempo fader
  - While change the tempo only of the left indicator is either off or of the color of the deck.
    - If green, it means the fader is out of sync with the software, bringing it down will eventually catch up.
    - If white, it means the fader is out of sync with the software, bringing it up will eventually catch up.
...
* - Rec
  - - Toggle the beatjump page and display jump option as defined in settings
  -
* - Sampler
  - - Toggle the sampler page and display samplers on the Using
  - - Off: Current page isn't related to sampler
    - On: sampler page is active
* - Mute
  - Currently unused
  -
* - Stems
  - - Toggle the stem page - Shift: toggle the keyboard (on release) while press: enable keyboard move mode
  - - Deck color with dim off: Current page isn't related to keyboard
    - Deck color with dim on: Stem page or Keyboard is active
    - Green: keyboard play mode active
* - Pads
  - - While in hotcue:
      - press will activate
      - if preview deck is playing the loaded track (using with {hwlabel}`SHIFT`) press will activate in preview deck
      - {hwlabel}`SHIFT` + press will delete (hotcue)
    - While in beatjump:
      - press will activate a jump forward of 1, 2, 4, 8, 16, 32, 64 or the currently select beatjump size, or custom size if you have changed _BeatJumpSize_ in {ref}`settings`
      - {hwlabel}`SHIFT` + press will perform the same jump backward
    - While in sample:
      - press will play (load selected track if none are)
      - {hwlabel}`SHIFT` + press will stop (if playing) or eject
    - While in stem:
      - the top row can be used to select a stem for further actions (FX assignation, FX dry/run, volume or pre-mixed load)
    ...
    - While in beatloop roll mode:
      - Will activate a beatloop roll of 1/16, 1/8, 1/4 , 1/2, 1, 2, 4 and 8 beats, or custom size if you have changed `BeatLoopRolls` in {ref}`settings`
  - - In hotcue: color of the cue. Blinking in delete (shift) mode
    - In beatjump: Green is defined jump, red is current jump value and white is selection edition
    ...
    - In Beatloop roll: brighter means a loop roll is active with the given size
* - FX 1 knob
  - - Master volume/mix of the unit
  -
* - FX 2|3|4 knob
  - - Meta arg of effect 1|2|3
    - Parameter 1|2|3 of the focused effect in effect focus mode
  -
* - FX 1 button
  - - Trigger all effects
    - Trigger all effect
    - Assign/de-assign effect unit to master while pressing {hwlabel}`SHIFT` and no focused effect
    - Exit focused mode while pressing {hwlabel}`SHIFT` and focused effect
  - - On if all effect are off and not pressing {hwlabel}`SHIFT`
    - On when effect is attached to master and pressing {hwlabel}`SHIFT`
    - Blinking in effect focused mode
* - {rspan}`3` FX 2|3|4 button
  - {cspan}`1` No effect focused
* - - Toggle (short press) or trigger (long press) effect
    - Hold {hwlabel}`SHIFT` and press to switch to next effect
  - On if active
* - {cspan}`1` Effect focus mode
* - - Hold {hwlabel}`SHIFT` and long-press to focus effect
    - Toggle (short press) or trigger (long press) parameter 1|2|3 of the focused effect
    - Toggle or trigger effect with {hwlabel}`SHIFT`
  - - On if effect parameter 1|2|3 is enabled
    - On when pressing {hwlabel}`SHIFT` and effect is active
* - Library knob
  ...
  -
* - Preview button
  - Preview the currently selected track while pressed Shift: Preview the currently playing track
  -
* - Star button
...
* - Mixer FX button
  - Toggle (short press) or trigger (long press) quick effect or assign the quick effect if an FX select button is pressed
  - Dim on if the effect is active
* - FX Select button
  - Apply effect to all deck on release. While pressed, press mixer FX button to assign effect to that deck
  -
* - Ext
  - Apply the current gain as default. This will reset the gain knob.
  -
* - Master
  - If enabled in the {ref}`settings`, change the main gain
  -
* - Booth
  - If enabled in the {ref}`settings`, change the booth gain
  -
* - Cue
  - If enabled in the {ref}`settings`, adjust the headphone mix
  -
* - Headphone volume
...
* - {hwlabel}`SHIFT` + {hwlabel}`REV` (while no loops are enabled)
  - Set the loop in point. This will reset the loop out point as well.
* - {hwlabel}`SHIFT` + {hwlabel}`REV` (while a loop is enabled)
  - Toggle the loop in jogwheel mode. See {ref}`jogwheel-modes` for more details.
* - {hwlabel}`SHIFT` + {hwlabel}`FLX` (while no loops are enabled)
  - Set the loop out point.
* - {hwlabel}`SHIFT` + {hwlabel}`FLX` (while a loop is enabled)
  - Toggle the loop out jogwheel mode. See {ref}`jogwheel-modes` for more details.
```
...
There are various options that can be used to change some behavior.<br>
...
* - Sortable column in the library
  - Artist, Title, BPM, Key, DatetimeAdded
  - Define the list of columns on which it possible to sort the library using the library encoder and the view button. See {mixxx:coref}`[Library],sort_column`
* - Loop In/Out jogwheel sensitivity
  - 50
  - Define the sensitivity when moving the loop start or end point using the loop jogwheel mode. Negative value will reverse the order
* - Loop encoder sensitivity
  - 500
  - Define the sensitivity when moving the loop with the encoder when using the loop jogwheel mode. Negative value will reverse the order
* - Loop encoder sensitivity (Shifted)
  - 2500
  - Define the sensitivity when moving the loop with {hwlabel}`SHIFT` + the encoder when using the loop jogwheel mode. Negative value will reverse the order
* - Tempo fader center range
  - 1.0
  - Defines the center range in mm where the rate snaps to 0.
* - Tempo fader center offset
  - 0.0
...
* - Keylock on {hwlabel}`SHIFT` + {hwlabel}`MASTER` instead of {hwlabel}`SHIFT` + {hwlabel}`SYNC`
  - false
  - Use {hwlabel}`SHIFT` + {hwlabel}`MASTER` to toggle the keylock instead of {hwlabel}`SHIFT` + {hwlabel}`SYNC`
* - Make the {hwlabel}`grid` button blink when over a detected beat
  - false
  - Make the {hwlabel}`GRID` button blinking when the playback goes over a detected beat
* - Make the jogwheel ring blink when the track playing is near the end
  - true
  - The jogwheel LED ring will start blinking when a track is near the end. The end section can be defined in {menuselection}`Preferences --> Waveforms --> End of track warning`
* - Use the mixer to control input when using {hwlabel}`SHIFT`
  - false
  - Make the {hwlabel}`GRID` button blinking when the playback goes over a detected beat
...
* - Use the two last tab as loop half/double buttons in the beatloop tab
  - true
  - Use the last two pad from the bottom row as loop half and loop double. These can be used to interact with beatloop roll and normal loop.
