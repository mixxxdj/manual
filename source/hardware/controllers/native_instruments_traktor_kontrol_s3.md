...
[driver from Native
Instruments](https://www.native-instruments.com/en/support/downloads/drivers-other-files/#traktorkontrols3)
...
:::{hint}
There are two {hwlabel}`SHIFT` buttons, one for each side of the
...
:::{sectionauthor} Sam Whited <sam@samwhited.com>
:::
Settings can be edited in the preference windows, under {guilabel}`Preferences` > {guilabel}`Controllers` > {guilabel}`Traktor Kontrol S3 ...`.
