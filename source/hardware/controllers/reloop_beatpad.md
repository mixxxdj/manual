![](../../_static/controllers/hardware/reloop/reloopbeatpadtp.png)
...
### PFL (symbolized by a headphone)
  ...
  - \*\*Slip mode: \*\*Hold **[SHIFT](#shift)** down and then press
    these buttons to activate/deactivate Slip Mode. When active, the
  ...
  - **Quantize mode:** Hold **[SHIFT](#shift)** down and then press
    these buttons to activate/deactivate the Quantize mode. When
...
### SYNC
  ...
  - **Press once** to synchronize the tempo (BPM) and phase to that of
    the other track.
  - **Press twice quickly** to play the track immediately, synchronized
  ...
  - **Sync Lock:** Hold for at least half of a second to enable **sync
    ...
    same tempo, and decks that also have **quantize** enabled (which is
...
```
 ...
 - **Sampler bank selection mode** (pink LED): each pad selects 1 of the 4 sampler banks. The PAD which is lit indicates which bank is active.
...
(work in progress) This section exposes several issues with the Reloop
