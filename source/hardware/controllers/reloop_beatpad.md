...
-  [Forum thread](https://mixxx.discourse.group/t/reloop-beatpad-ready-for-1-12/15408)
...
Adjust the Volume of each channel. If {ref}`Fader Start <reloop-beatpad-trackselect>`
...
:::{hint}
Change the default cue mode in Preferences -> Interface. The
...
(reloop-beatpad-seekmode)=

#### Seek mode


Toggle with the **JOG SEEK** button.<br>
Use the jogwheel to navigate through the track.


(reloop-beatpad-cdmode)=

#### CD mode


Deactivate both the {ref}`Scratch mode <reloop-beatpad-scratchmode>` and the {ref}`Seek mode <reloop-beatpad-seekmode>` to enable this mode.<br>
...
:::{seealso}
Rolling loops can be set using the {ref}`Bounce Loop (Roll) mode <reloop-beatpad-rollmode>`.
:::
(reloop-beatpad-effects)=
...
-  **Current effect rack selection:** Hold {ref}`SHIFT <reloop-beatpad-shift>` down and then
...
-   **Effect rack selection** mode (purple LED): each pads selects 1 of the 4 effect racks. The PAD which is lit indicates which effect rack is active (see Grounding Problem`SHIFT <reloop-beatpad-shift>` + {ref}`Sampler <reloop-beatpad-samplermode>`:** On account of
