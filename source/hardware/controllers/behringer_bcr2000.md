...
1. Load the dump file ``bcr_Only_Controllers.syx`` via Sysex to any other preset slot and update
...
3. Hold {hwlabel}`EDIT`, press a button, then release both
4. Turn the {hwlabel}`MODE` encoder until the display shows ``toFF``
5. Press the {hwlabel}`EXIT` button
...
* - All controls
  - Button 4
  - {rspan}`1`
    - Press: Toggles shift. Shift changes the behaviour of controls as described in this table, and additionally the behaviour of the effect units. See {ref}`Standard Effects Mapping <controller-effects-mapping>` for details.
    :::{note}
    Both Shift buttons have the same effect, they are not deck-specific. This design decision was made to keep the layout symmetric so that you don't have to press a button on the left side when you're working on the right side.
    :::
* - All controls
  - Button 8
* - {rspan}`7` Button Row 2
  - Effect Unit 1
  - Button 1
  - {rspan}`1`
