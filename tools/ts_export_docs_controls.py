...
from docutils.nodes import Node, document, field_list
...
def read_pot_meter_suffixes(doc: document) -> list[PotMeterSuffix]:
    ...
    return [
        PotMeterSuffix(cpms[0], cpms[1]) for cpms in read_table(cpms_table)[1:]
    ]
...
def read_group_info(doc: document) -> list[Group]:
    tags = filter(
        lambda n: "objtype" in n.attributes and n["objtype"] == "controlgroup",
        doc.findall(addnodes.desc),
    )
    return list(map(extract_group_info, tags))
...
def create_control_types_str(
    ...
    for group, controls in sorted(groupControls.items()):
        ...
        lines.extend(
            [
                f"| {prefix}{group_var(p)}"
                for p in find_group_parents(group, ctls)
            ]
        )
        lines.append(";\n")
...
def get_group(name: str, groups: list[Group]) -> Group:
    try:
...
def export_ts_types(
    output: Path,
    group_info: list[Group],
    controls: list[Control],
    pm_suffixes: list[PotMeterSuffix],
):
    ...
    with open(output, "w") as f:
        f.write(
            f"""// Mixxx control types
// Generated file, don't change by hand, will be overwritten

declare namespace MixxxControls {{
{"\n".join(create_group_types_str(group_info))}
...
if __name__ == "__main__":
    ...
    for section in doc.findall(nodes.section):
        if (
            "removed-controls" in section["ids"]
            or "mixxx-controls" in section["ids"]
        ):
            continue

        for node in section.findall(addnodes.desc):
            if (
                "mixxx" not in node["classes"]
                or "control" not in node["classes"]
            ):
                continue
